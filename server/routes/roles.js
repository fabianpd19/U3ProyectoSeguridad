...
router.put("/:id", requirePermission("roles", "update"), async (req, res) => {
  try {
    ...
    const duplicateRole = await executeQuery(
      ...
      [sanitizedData.name, roleId]
    );

    if (duplicateRole.length > 0) {
      return res
        .status(400)
        .json({ error: "Ya existe otro rol con ese nombre" });
    }
  ...
  async (req, res) => {
    try {
      ...
      if (systemRoles.includes(existingRole[0].name)) {
        return res
          .status(400)
          .json({ error: "No se puede eliminar un rol del sistema" });
      }
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const role = await executeQuery("SELECT name FROM roles WHERE id = ?", [
        roleId,
      ]);
      ...
      if (existingAssignment.length > 0) {
        return res
          .status(400)
          .json({ error: "El usuario ya tiene este rol asignado" });
      }
  ...
  async (req, res) => {
    ...
    `,
      ...
      if (assignment.length === 0) {
        return res
          .status(404)
          .json({ error: "Asignación de rol no encontrada" });
      }
      ...
      if (assignment[0].role_name === "admin") {
        ...
        if (adminCount[0].count <= 1) {
          return res
            .status(400)
            .json({ error: "No se puede remover el último administrador" });
        }
      }
